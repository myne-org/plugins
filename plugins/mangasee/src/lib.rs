...
pub fn search(query: &str) -> FnResult<Json<Vec<SearchResult>>> {
    ...
    page.navigate_to(
        Url::parse_with_params(format!("{BASE_URL}/search/").as_str(), [("name", query)])?
            .as_str(),
    )?
    .wait_until_navigated()?;

    let doc = Html::parse_document(&page.get_content()?);
    let res: Vec<_> = doc.select(&Selector::parse(".top-15.ng-scope").unwrap()).map(|s| {
        let first = s.select(&Selector::parse(r#".SeriesName[ng-bind-html="Series.s"]"#).unwrap()).next().unwrap();
